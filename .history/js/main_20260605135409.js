...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (track && prevBtn && nextBtn) {
        ...
        let cardWidth, visibleCards, maxSlide;

        function calcCarouselDimensions() {
            const gap = parseInt(getComputedStyle(track).gap) || 24;
            cardWidth = cards[0]?.offsetWidth + gap || 400;
        ...
        track.addEventListener('touchmove', (e) => {
            ...
            track.style.transform = `translateX(${-currentSlide * cardWidth + dragOffset}px)`;
        }, { passive: true });
        track.addEventListener('touchend', () => {
            isDragging = false;
            if (Math.abs(dragOffset) > cardWidth * 0.25) {
